...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'weather':
                return await weather.getWeather(args?.location);
            case 'qr_code':
                return await qrcode.generate(args?.text, args?.size);
            case 'color_info':
                if (args?.action === 'scheme') {
                    return await color.generateScheme(args?.hex, args?.mode);
                }
                else {
                    return await color.identify(args?.hex);
                }
            case 'shorten_url':
                return await url.shorten(args?.url);
            case 'generate_uuid':
            ...
            case 'fake_data':
                return await fake.generate(args?.type || 'persons', args?.count || 3);
            case 'placeholder_image':
                return await placeholder.generate(args?.width || 800, args?.height || 600);
            case 'terminal_power_workflow':
                return await executeWorkflow(args?.commands);
            default:
...
async function executeWorkflow(commands) {
    const results = [];
    for (const cmd of commands) {
        const result = await server.request({ method: 'tools/call', params: { name: cmd.tool, arguments: cmd.params } }, CallToolRequestSchema);
        results.push(result);
    }
    return {
        content: [
