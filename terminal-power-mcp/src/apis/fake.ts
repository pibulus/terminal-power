...
export class FakeDataAPI {
  async generate(type: string = 'persons', count: number = 3) {
    try {
      const response = await fetch(
        `https://fakerapi.it/api/v1/${type}?_quantity=${count}`
      );
      ...
      const preview = data.data.slice(0, 2).map((item: any, index: number) => {
        if (type === 'persons') {
